...
int main(int argc, char **argv)
{
	...
	if ((ret=parseIFFImage(&gfx, f)) != IFF_NO_ERROR){
		...
		if (screen){
			...
			//img = createBitMap(&gfx, appWnd->appWindow->RPort->BitMap);
			img = createBitMap(&gfx, NULL);
			if (img){
				if (myApp.isScreenRTG){
					cs = createColorMap(&gfx,8);
					if(cs){
